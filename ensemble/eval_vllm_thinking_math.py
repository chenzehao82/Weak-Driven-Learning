...
def load_general_dataset(path: str) -> List[Dict[str, Any]]:
    ...
    for i, ex in enumerate(data):
        ## Try multiple possible question field names
...
## -----------------------------
## Simple number/letter extraction method (for simple math datasets)
## -----------------------------
def extract_answer_number(sentence: str) -> float:
    """Extract numeric answer from text (take the last number)"""
...
def extract_answer_letter(sentence: str) -> str:
    """Extract letter answer from text (A-E)"""
...
## -----------------------------
## Latex-based Verification (for complex math datasets)
## -----------------------------
def verify_with_latex(idx: str, pred: str, gold: str) -> Optional[bool]:
    try:
        ...
        if len(gold_parsed) == 0:
            print(f"Failed to parse gold solution: {gold}")
            return None
...
def should_use_chat_template(dataset_name: str) -> bool:
    """
    Determine whether to use chat_template based on dataset name
    Simple datasets (e.g., math_10k, gsm8k) don't need chat_template
    Complex datasets (e.g., aime, math500) need chat_template
    """
    dataset_name = dataset_name.lower()
    
    ## Datasets that don't need chat_template (usually simple math problems)
    simple_datasets = [
        "multiarith", "addsub", "singleeq", "gsm8k", "svamp", "mawps", 
        "math_10k", "aqua"
    ]
    ...
    ## Other datasets (aime, math500, etc.) use chat_template
    return True
...
def evaluate(dataset_path: str,
    ...
    for ex, text, lat in zip(expanded_samples, preds, latencies):
        ...
        rows.append({
            "id": ex["id"],
            "orig_id": ex["id"].split("_rep")[0],   ## ⭐ Sample id（）
            "gt": ex["answer"],
...
def main():
    ...
    p.add_argument("--repeat", type=int, default=3, help="Number of test repetitions")
    p.add_argument("--epoch", type=str, default=None, help="Number of training epochs")
    p.add_argument("--dataset_name", type=str, default=None, help="Datasetname（Save）")
    ...
    ## （baseline/qwen2.5-7b/checkpoint-872）
    ##  weights/  EnsembleLLM/weights/ 
